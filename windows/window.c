...
static void close_session(void)
{
	...
	DeleteMenu(popup_menus[i].menu, IDM_RESTART, MF_BYCOMMAND);
	InsertMenu(popup_menus[i].menu, IDM_DUPSESS, MF_BYCOMMAND | MF_ENABLED,
		   IDM_RESTART, "&重启会话\t[Enter]/[Ctrl+Shift+S]");
    DeleteMenu(popup_menus[i].menu, IDM_STOP, MF_BYCOMMAND);
    }
...
int WINAPI WinMain(HINSTANCE inst, HINSTANCE prev, LPSTR cmdline, int show)
{
    ...
    AppendMenu(find_menu, MF_ENABLED, IDM_FIND, "&查找\t[Ctrl+Shift+F]");
    AppendMenu(find_menu, MF_SEPARATOR, 0, 0);
    AppendMenu(find_menu, MF_ENABLED, IDM_FIND_PREV, "&查找上一个\t[F3]");
    AppendMenu(find_menu, MF_ENABLED, IDM_FIND_NEXT, "&查找下一个\t[F4]");
	...
	for (j = 0; j < lenof(popup_menus); j++) {
	    ...
	    AppendMenu(m, MF_SEPARATOR, 0, 0);
	    AppendMenu(m, MF_ENABLED, IDM_COPYALL, "复制所有内容到剪贴板(&O)\t[Ctrl+Shift+A]");
	    AppendMenu(m, MF_ENABLED, IDM_CLRSB, "清除滚动条(&L)");
	    AppendMenu(m, MF_ENABLED, IDM_RESET, "清除屏幕(&T)");
	    AppendMenu(m, MF_ENABLED, IDM_CLRSS, "清除屏幕并回滚\t[Ctrl+Shift+Z]");
//	    AppendMenu(m, MF_ENABLED, IDM_FIND, "查找(&F)");
	    AppendMenu(m, MF_POPUP | MF_ENABLED, (UINT)find_menu, "查找");
	    AppendMenu(m, MF_SEPARATOR, 0, 0);
	    AppendMenu(m, (conf_get_int(conf, CONF_resize_action)
			   == RESIZE_DISABLED) ? MF_GRAYED : MF_ENABLED,
		       IDM_FULLSCREEN, "全屏(&F)");
	    AppendMenu(m, MF_SEPARATOR, 0, 0);
	    AppendMenu(m, term->xyz_transfering?MF_GRAYED:MF_ENABLED, IDM_XYZSTART, "Zmodem 接收(&Z)");
	    AppendMenu(m, term->xyz_transfering?MF_GRAYED:MF_ENABLED, IDM_XYZUPLOAD, "Zmodem 发送(&U)");
	    AppendMenu(m, !term->xyz_transfering?MF_GRAYED:MF_ENABLED, IDM_XYZABORT, "Zmodem 终止(&A)");
	    AppendMenu(m, MF_SEPARATOR, 0, 0);
	    if (has_help())
